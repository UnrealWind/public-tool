function FormLogic(tags,groups,$target,isOpt){
  init();
  ...
  function genDetailHtml(sourceData,value,ischeck) {
    var childHtml = "";
    childHtml += '<div unique="' + sourceData.uniqueId + '" class="infi-logic-detail">' +
        '<span class="infi-logic-detail-label">' + sourceData.tagLabel + '</span>:<span class="glyphicon glyphicon-minus-sign remove-detail"></span>' +
        '<select class="form-control infi-input-w-xsx infi-logic-detail-equal" value="' + sourceData.isEqual + '" '+isOpt+'>' +
        '<option value="yes" ' + ischeck.yes + '>等于</option>' +
        '<option value="no" ' + ischeck.no + '>不等于</option>' +
        '</select>';
    childHtml += '<span class="infi-logic-detail-value" unique="' + value.uniqueId+'">' + value.label +'</span></div>&nbsp&nbsp';
    return childHtml;
  }
  ...
  function getValueData(id,logicDetailData) {
    angular.forEach(tags,function (tag) {
      if(logicDetailData.uniqueId == tag.uniqueId){
  ...
  function drop(e) {
    ...
    while (targetElem) {
      ...
      for (var idx = 0; idx < recordData.length; idx++) {
        //拖动的是姓名类型的
        if (recordData[idx].uniqueId == sourceElem.attr("id")) {
          ...
          //拖动的是男、女类型的,唯一标示为uniqueId
          for (var idy = 0; idy < recordData[idx].values.length; idy++) {
            if (recordData[idx].values[idy].uniqueId == sourceElem.attr("id")) {
