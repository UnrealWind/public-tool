angular.module('infi-basic')
    .controller('ViewController', ['$scope','ViewService','SYS',function ($scope,ViewService,SYS) {
        $scope.SYS = SYS;
        //用于显示左侧tab的数据
        $scope.summaryData = {};
        //用于显示右侧性别、年龄等信息
        ...
        function init(){
            ...
            ViewService.getSummaryList().then(function (msg) {
    ...
    }]).service("ViewService",['$http','SYS',function ($http,SYS) {
    /**
     * 调用左侧tab的数据
     * @returns {*}
     */
    function getSummaryList(filter){
        return $http.get(SYS.url+"tabs.json").then(function success(msg){
            return msg.data;
        })
    }
