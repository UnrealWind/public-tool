...
dwv.html.Layer = function(canvas)
{
    ...
    this.draw = function ()
    {
        ...
        //水印的字体大小与位置与上传的dicon图比例有关,需要进行计算,否则无法正常显示
        var fontSize = Math.round(15/zoom.y)+'px',
            tranX = Math.round(100/zoom.x),
            tranY = Math.round(200/zoom.x);
        context.font=fontSize+" microsoft yahei";
        context.rotate(-20*Math.PI/180);
        context.fillStyle = "rgba(255,255,255,0.5)";
        context.fillText(dwv.watermark,tranX,tranY);

        //当图形绘制完成后再显示,未完成时显示提示文字
        $("#dwv").css("visibility","visible");
        $("#loadTip").text("");
    };
