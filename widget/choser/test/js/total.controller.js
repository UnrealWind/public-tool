angular.module('infi-basic')
    .controller('TotalController', ['$scope','TotalService','SYS',function ($scope,TotalService,SYS) {
        $scope.SYS = SYS;
        //用于显示左侧tab的数据
        $scope.summaryData = {};
        //用于显示右侧性别、年龄等信息
        ...
        function init(){
            ...
            TotalService.getSummaryList().then(function (msg) {
    ...
    }]).service("TotalService",['$http','SYS',function ($http,SYS) {
    /**
     * 调用左侧tab的数据
     * @returns {*}
     */
    function getSummaryList(filter){
        return $http.get(SYS.url+"total.json").then(function success(msg){
            return msg.data;
        })
    }
